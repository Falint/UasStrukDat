...
public:
    ...
    vector<Ticket>* cari(const string& nama) {
        ...
        for (auto& node : table[index]) {
            if (toLowerCase(node.nama) == toLowerCase(nama)) {
    ...
    bool hapus(const string& nama) {
        ...
        while (it != table[index].end()) {
            if (toLowerCase(it->nama) == toLowerCase(nama)) {
    ...
    void tampilStatistik() {
        ...
        for (int i = 0; i < TABLE_SIZE; i++) {
            if (!table[i].empty()) {
                bucketTerisi++;
                totalNode += table[i].size();
                
                if (table[i].size() > 1) {
                    totalCollision += table[i].size() - 1;
                }
            }
        }
        
        cout << "\n=== Statistik Hash Table ===\n";
        cout << "Jumlah Bucket     : " << TABLE_SIZE << endl;
        cout << "Bucket Terisi     : " << bucketTerisi << endl;
        cout << "Total Penumpang   : " << totalNode << endl;
        cout << "Total Collision   : " << totalCollision << endl;
        cout << "Load Factor       : " << (double)totalNode / TABLE_SIZE << endl;
    }
...
void menu() {
    ...
    do {
        ...
        cout << "4. Edit Tiket\n";
        cout << "5. Hapus Tiket\n";
        cout << "6. Statistik Hash Table\n";
        cout << "7. Sorting Tiket\n";
        cout << "0. Keluar\n";
        cout << "Pilih menu: ";
